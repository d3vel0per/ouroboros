...
Connects to the Ouroboros MCP server via stdio, invokes
``ouroboros_evolve_step`` (and optionally ``ouroboros_lateral_think``
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--server-command",
        default="ouroboros",
        help="MCP server executable (default: ouroboros)",
    )
    p.add_argument(
        "--server-args",
